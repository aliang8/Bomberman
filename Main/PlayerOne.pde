  ...
  void action() {
    if (s.name.equals("red") && s.curMove != "dying") {
      if (downKeys['w'] && s.curMove.equals("") || s.curMove.equals("walkUp")) {
        if (grid[(s.y - 1)/per][(s.x + 24)/per] == 1 || grid[(s.y - 1)/per][(s.x - 1)/per] == 1)
        //|| grid[(s.y - 3)/per][s.x/per] == 2 || grid[(s.y - 3)/per][s.x/per] == 3 || grid[(s.y - 3)/per][s.x/per] == 4) {
          {
          s.y += 3;
        }
        s.dir = 'u';
        ...
        if (grid[(s.y + 44)/per][(s.x - 3)/per] == 1 || grid[(s.y + 4)/per][(s.x - 3)/per] == 1) {
        //|| grid[s.y/per][(s.x - 3)/per] == 2 || grid[s.y/per][(s.x - 3)/per] == 3 || grid[s.y/per][(s.x - 3)/per] == 4) asa{
          s.x += 3;
        }
        s.dir = 'l';
        s.walkMove(10, 14, "walkLeft");
      } else if (downKeys['s'] && s.curMove.equals("") || s.curMove.equals("walkDown")) {
        if (grid[(s.y + 47)/per][(s.x + 12)/per] == 1 || grid[(s.y + 47)/per][(s.x - 1)/per] == 1) {
        //|| grid[(s.y + 50)/per][s.x/per] == 2 || grid[(s.y + 50)/per][s.x/per] == 3 || grid[(s.y + 50)/per][s.x/per] == 4) {
          s.y -= 3;
        }
        s.dir = 'd';
        s.walkMove(5, 9, "walkDown");
      } else if (downKeys['d'] && s.curMove.equals("") || s.curMove.equals("walkRight")) {
        if (grid[(s.y + 44)/per][(s.x + 27)/per] == 1 || grid[(s.y + 4)/per][(s.x + 27)/per] == 1) {
          //|| grid[s.y/per][(s.x + 25)/per] == 2 || grid[s.y/per][(s.x + 25)/per] == 3 || grid[s.y/per][(s.x + 25)/per] == 4) {
          s.x -= 3;
        }
        s.dir = 'r';
